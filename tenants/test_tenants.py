...
class TestTenantViews:
    ...
    def test_list_tenants(self, authenticated_seller_client, seller_tenant):
        """Test listing tenants"""
        response = authenticated_seller_client.get("/api/tenants/tenants/")
        assert response.status_code == status.HTTP_200_OK
        assert len(response.data) >= 1

    def test_retrieve_tenant(self, authenticated_seller_client, seller_tenant):
        """Test retrieving a tenant"""
        response = authenticated_seller_client.get(f"/api/tenants/tenants/{seller_tenant.id}/")
        assert response.status_code == status.HTTP_200_OK
        assert response.data["name"] == seller_tenant.name

    def test_create_tenant(self, authenticated_seller_client):
        """Test creating a tenant"""
        data = {"name": "New Tenant", "type": "buyer", "email": "newtenant@example.com", "phone": "999-999-9999"}
        response = authenticated_seller_client.post("/api/tenants/tenants/", data)
        assert response.status_code == status.HTTP_201_CREATED
        assert response.data["name"] == "New Tenant"

    def test_update_tenant(self, authenticated_seller_client, seller_tenant):
        """Test updating a tenant"""
        data = {"name": "Updated Seller", "type": "seller", "email": seller_tenant.email}
        response = authenticated_seller_client.put(f"/api/tenants/tenants/{seller_tenant.id}/", data)
        assert response.status_code == status.HTTP_200_OK
        assert response.data["name"] == "Updated Seller"

    def test_partial_update_tenant(self, authenticated_seller_client, seller_tenant):
        """Test partial update of tenant"""
        data = {"phone": "111-111-1111"}
        response = authenticated_seller_client.patch(f"/api/tenants/tenants/{seller_tenant.id}/", data)
        assert response.status_code == status.HTTP_200_OK
        assert response.data["phone"] == "111-111-1111"

    def test_delete_tenant(self, authenticated_seller_client):
        """Test deleting a tenant"""
        tenant = Tenant.objects.create(name="Delete Me", type="buyer", email="delete@example.com")
        tenant_id = tenant.id
        response = authenticated_seller_client.delete(f"/api/tenants/tenants/{tenant_id}/")
        assert response.status_code == status.HTTP_204_NO_CONTENT
        assert not Tenant.objects.filter(id=tenant_id).exists()

    def test_tenant_addresses_action(self, authenticated_seller_client, seller_tenant, tenant_address):
        """Test custom addresses action on tenant"""
        response = authenticated_seller_client.get(f"/api/tenants/tenants/{seller_tenant.id}/addresses/")
        assert response.status_code == status.HTTP_200_OK
        assert len(response.data) >= 1
        assert response.data[0]["address1"] == tenant_address.address1

    def test_tenant_distributors_action_success(self, authenticated_seller_client, seller_tenant, distributor_tenant):
        ...
        response = authenticated_seller_client.get(f"/api/tenants/tenants/{seller_tenant.id}/distributors/")
        assert response.status_code == status.HTTP_200_OK
        assert len(response.data) >= 1

    def test_tenant_distributors_action_non_seller(self, authenticated_seller_client, buyer_tenant):
        """Test distributors action on non-seller tenant"""
        response = authenticated_seller_client.get(f"/api/tenants/tenants/{buyer_tenant.id}/distributors/")
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert "Only sellers have distributors" in response.data["error"]

    def test_list_tenant_addresses(self, authenticated_seller_client, tenant_address):
        """Test listing tenant addresses"""
        response = authenticated_seller_client.get("/api/tenants/addresses/")
        assert response.status_code == status.HTTP_200_OK
        assert len(response.data) >= 1

    def test_retrieve_tenant_address(self, authenticated_seller_client, tenant_address):
        """Test retrieving a tenant address"""
        response = authenticated_seller_client.get(f"/api/tenants/addresses/{tenant_address.id}/")
        assert response.status_code == status.HTTP_200_OK
        assert response.data["address1"] == tenant_address.address1

    def test_create_tenant_address(self, authenticated_seller_client, seller_tenant):
        ...
        response = authenticated_seller_client.post("/api/tenants/addresses/", data)
        assert response.status_code == status.HTTP_201_CREATED
        assert response.data["address_type"] == "billing"

    def test_update_tenant_address(self, authenticated_seller_client, tenant_address):
        ...
        response = authenticated_seller_client.put(f"/api/tenants/addresses/{tenant_address.id}/", data)
        assert response.status_code == status.HTTP_200_OK
        assert response.data["city"] == "Boston"

    def test_partial_update_tenant_address(self, authenticated_seller_client, tenant_address):
        """Test partial update of tenant address"""
        data = {"city": "San Francisco"}
        response = authenticated_seller_client.patch(f"/api/tenants/addresses/{tenant_address.id}/", data)
        assert response.status_code == status.HTTP_200_OK
        assert response.data["city"] == "San Francisco"

    def test_delete_tenant_address(self, authenticated_seller_client, seller_tenant):
        ...
        response = authenticated_seller_client.delete(f"/api/tenants/addresses/{address_id}/")
        assert response.status_code == status.HTTP_204_NO_CONTENT
        assert not TenantAddress.objects.filter(id=address_id).exists()

    def test_list_tenant_associations(self, authenticated_seller_client, tenant_association):
        """Test listing tenant associations"""
        response = authenticated_seller_client.get("/api/tenants/associations/")
        assert response.status_code == status.HTTP_200_OK
        assert len(response.data) >= 1

    def test_retrieve_tenant_association(self, authenticated_seller_client, tenant_association):
        """Test retrieving a tenant association"""
        response = authenticated_seller_client.get(f"/api/tenants/associations/{tenant_association.id}/")
        assert response.status_code == status.HTTP_200_OK
        assert str(response.data["seller"]) == str(tenant_association.seller.id)

    def test_create_tenant_association(self, authenticated_seller_client, seller_tenant, buyer_tenant):
        """Test creating a tenant association"""
        data = {"seller": str(seller_tenant.id), "buyer": str(buyer_tenant.id)}
        response = authenticated_seller_client.post("/api/tenants/associations/", data)
        assert response.status_code == status.HTTP_201_CREATED

    def test_update_tenant_association(self, authenticated_seller_client, tenant_association, seller_tenant, distributor_tenant):
        """Test updating a tenant association"""
        data = {"seller": str(seller_tenant.id), "buyer": str(distributor_tenant.id), "is_active": False}
        response = authenticated_seller_client.put(f"/api/tenants/associations/{tenant_association.id}/", data)
        assert response.status_code == status.HTTP_200_OK
        assert response.data["is_active"] is False

    def test_partial_update_tenant_association(self, authenticated_seller_client, tenant_association):
        """Test partial update of tenant association"""
        data = {"is_active": False}
        response = authenticated_seller_client.patch(f"/api/tenants/associations/{tenant_association.id}/", data)
        assert response.status_code == status.HTTP_200_OK
        assert response.data["is_active"] is False

    def test_delete_tenant_association(self, authenticated_seller_client, seller_tenant, buyer_tenant):
        ...
        response = authenticated_seller_client.delete(f"/api/tenants/associations/{association_id}/")
        assert response.status_code == status.HTTP_204_NO_CONTENT
